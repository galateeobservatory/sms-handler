...
impl E3372 {
    ...
    pub fn get_sms_list(&self) -> String {
        ...
        // boxtype = 1 -> recv
        // boxtype = 2 -> sent
        let mut res = self._client.post("http://192.168.8.1/api/sms/sms-list").header(CUSTOM_HEADER, csrf_token.clone()).body(r#"<?xml version: "1.0" encoding="UTF-8"?><request><PageIndex>1</PageIndex><ReadCount>20</ReadCount><BoxType>2</BoxType><SortType>0</SortType><Ascending>0</Ascending><UnreadPreferred>0</UnreadPreferred></request>"#).send().unwrap();
        let mut body = String::from("");
        res.read_to_string(&mut body).unwrap();
        return body;
    }
    ...
    fn extract_csrf_token(body: &String) -> String {
